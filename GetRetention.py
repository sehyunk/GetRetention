...
def get_retention(df):
    cohort_nunique = get_cohort_nunique(df)
    retention = pd.DataFrame()
    retention.loc['retention', 0] = 100

    for i in cohort_nunique.columns[1:]:
        i_cohort = cohort_nunique.loc[cohort_nunique[i].notna(),[0,i]].sum()
        ret_ratio = i_cohort[i]/i_cohort[0]*100
        retention.loc['retention', i] = ret_ratio
